...
const { initialPreviewLines, loadSavedPreviewLines, savePreviewLines } =
	await import(
...
if (mode === "write") {
	check(
		"no state file + no env -> built-in default",
		initialPreviewLines() === 3,
		`N=${initialPreviewLines()}`,
	);
	savePreviewLines(7);
	check(
		"saved N round-trips",
		loadSavedPreviewLines() === 7,
		`read ${loadSavedPreviewLines()}`,
	);
} else if (process.env.PI_THINKING_PREVIEW_LINES) {
	check(
		"env beats the saved state",
		initialPreviewLines() === 9,
		`N=${initialPreviewLines()}`,
	);
} else {
	check(
		"saved state beats the built-in default",
		initialPreviewLines() === 7,
		`N=${initialPreviewLines()}`,
	);
}
