...
for (const [shape, text] of Object.entries(shapeOf)) {
	for (const width of [100, 60, 34, 20]) {
		...
		for (let n = 1; n <= 12; n++) {
			...
			const hint = hintRowCount(rendered);
			const reasoning = reasoningRowCount(rendered);
			// A block that fits the budget is shown whole, so it may be shorter than N.
			if (hint === 0 ? reasoning > n : reasoning !== n)
				wrong.push(
					`N=${n}->${reasoning} reasoning of ${rendered.length}, hint ${hint}`,
				);
		}
		check(
			`${shape} at width ${width}: every budget renders exactly N reasoning rows under the hint`,
			wrong.length === 0,
			wrong.join(" "),
		);
	}
